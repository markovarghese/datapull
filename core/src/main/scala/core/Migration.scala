...
class Migration extends SparkListener {
  ...
  def migrate(migrationJSONString: String, reportEmailAddress: String, migrationId: String, verifymigration: Boolean, reportCounts: Boolean, no_of_retries: Int, custom_retries: Boolean, migrationLogId: String, isLocal: Boolean, preciseCounts: Boolean, appConfig: AppConfig, pipeline: String): Map[String, String] = {
    ...
    try {
      ...
      if (destinationMap("platform") == "cassandra") {
        ...
        dataframeFromTo.dataFrameToMongodb(destinationMap("awsenv"), destinationMap("cluster"), destinationMap("database"), destinationMap.get("authenticationdatabase"), destinationMap("collection"), destinationMap.get("login"), destinationMap.get("password"), destinationMap.get("replicaset"), destinationMap.getOrElse("replacedocuments", "true").toBoolean, destinationMap.getOrElse("orderedrecords", "false").toBoolean, dft, sparkSession, destinationMap.getOrElse("documentfromjsonfield", "false").toBoolean, destinationMap.getOrElse("jsonfield", "jsonfield"), destinationMap("vaultenv"), destinationMap.getOrElse("secretstore", "vault"), destination.optJSONObject("sparkoptions"), if (destinationMap.get("maxBatchSize").isEmpty) None else Some(destinationMap.getOrElse("maxBatchSize", 0).toString.toLong), destinationMap.getOrElse("sslenabled", "false").toBoolean)
  ...
  def jsonSourceDestinationToDataFrame(sparkSession: org.apache.spark.sql.SparkSession, platformObject: JSONObject, migrationId: String, jobId: String, s3TempFolderDeletionError: StringBuilder, pipeline: String): org.apache.spark.sql.DataFrame = {
    ...
    if (platform == "mssql" || platform == "mysql" || platform == "oracle" || platform == "postgres" || platform == "teradata") {
      ...
      dataframeFromTo.mongodbToDataFrame(propertiesMap("awsenv"), propertiesMap("cluster"), propertiesMap.getOrElse("overrideconnector", "false").toBoolean, propertiesMap("database"), propertiesMap.get("authenticationdatabase"), propertiesMap("collection"), propertiesMap.get("login"), propertiesMap.get("password"), propertiesMap.get("replicaset"), sparkSession, propertiesMap("vaultenv"), platformObject.optJSONObject("sparkoptions"), propertiesMap.getOrElse("secretstore", "vault"), propertiesMap.getOrElse("tmpfilelocation", null), if (propertiesMap.get("samplesize").isEmpty) None else Some(propertiesMap.getOrElse("samplesize", 0).toString.toLong), propertiesMap.getOrElse("sslenabled", "false").toBoolean)
    }
    ...
    if ((runPreMigrationCommand && (pre_migrate_commands.length() > 0)) || ((!runPreMigrationCommand) && (post_migrate_commands.length() > 0))) {
      ...
      for (i <- 0 to lengthOfArray - 1) {
        ...
        if (platform == "mssql" || platform == "mysql" || platform == "oracle" || platform == "postgres" || platform == "teradata") {
          ...
          dataframeFromTo.mongoRunCommand(awsEnv = propertiesMap("awsenv"), cluster = propertiesMap("cluster"), database = propertiesMap("database"), authenticationDatabase = propertiesMap.get("authenticationdatabase"), collection = propertiesMap("collection"), login = propertiesMap.get("login"), password = propertiesMap.get("password"), vaultEnv = propertiesMap("vaultenv"), addlSparkOptions = platformObject.optJSONObject("sparkoptions"), runCommand = command.getJSONObject(i).getString("query"), secretStore = propertiesMap.getOrElse("secretstore", "vault"), propertiesMap.getOrElse("sslenabled", "false").toBoolean)
        } else if (platform == "elastic") {
          propertiesMap = propertiesMap ++ deriveClusterIPFromConsul(jsonObjectPropertiesToMap(List("clustername", "cluster_key", "consul_dc"), platformObject))
